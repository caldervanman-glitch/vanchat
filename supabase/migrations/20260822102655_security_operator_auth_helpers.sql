-- Security remediation for PR #1 findings 1 and 3.
-- Privileged QA/refinery callers keep the Supabase gateway credential and add
-- a separate operator token sourced from Vault at runtime. No raw operator
-- credential is committed to GitHub.
...
declare
  ...
  rid bigint;
begin
  select decrypted_secret into k from vault.decrypted_secrets where name='qa_runner_publishable_key' limit 1;
  select decrypted_secret into op from vault.decrypted_secrets where name='vanhub_operator_token' limit 1;
  if coalesce(k,'')='' then raise exception 'QA runner credential is not configured'; end if;
  if coalesce(op,'')='' then raise exception 'VanHub operator credential is not configured'; end if;
  select net.http_post(
    url := 'https://adpphssfoxpyzzonqofz.supabase.co/functions/v1/vanhub-chat-expectation-runner',
    headers := jsonb_build_object('Content-Type','application/json','Authorization','Bearer '||k,'x-vanhub-operator-token',op),
...
begin
  ...
  select net.http_post(
    url:='https://adpphssfoxpyzzonqofz.supabase.co/functions/v1/vanhub-vector-distill',
    body:=jsonb_build_object('action','process_part','file_id',p_file_id,'part_index',p_part_index),
    headers:=jsonb_build_object('Content-Type','application/json','apikey',v_key,'x-vanhub-operator-token',v_op),
...
begin
  ...
  if coalesce(v_op,'')='' then raise exception 'VANHUB_OPERATOR_KEY_MISSING'; end if;
  select net.http_post(
    url:='https://adpphssfoxpyzzonqofz.supabase.co/functions/v1/vanhub-refinery-adjudicate',
    body:=jsonb_build_object('action','adjudicate_domain','domain',p_domain),
    headers:=jsonb_build_object('Content-Type','application/json','apikey',v_key,'x-vanhub-operator-token',v_op),
    timeout_milliseconds:=120000
  ) into v_request_id;
  return v_request_id;
...
begin
  ...
  select * into v_resp from extensions.http((
    ...
    'https://adpphssfoxpyzzonqofz.supabase.co/functions/v1/vanhub-refinery-adjudicate'::varchar,
    array[extensions.http_header('Content-Type','application/json'),extensions.http_header('apikey',v_key),extensions.http_header('x-vanhub-operator-token',v_op)],
    'application/json'::varchar,
    jsonb_build_object('action','adjudicate_domain','domain',p_domain)::text
...
begin
  ...
  if coalesce(v_op,'')='' then raise exception 'VANHUB_OPERATOR_KEY_MISSING'; end if;
  select net.http_post(
    url:='https://adpphssfoxpyzzonqofz.supabase.co/functions/v1/vanhub-vector-audit-fallback',
    body:=jsonb_build_object('file_id',p_file_id,'start',greatest(0,p_start),'limit',least(20000,greatest(1,p_limit))),
    headers:=jsonb_build_object('Content-Type','application/json','apikey',v_key,'x-vanhub-operator-token',v_op),
    timeout_milliseconds:=120000
  ) into v_request_id;
  return v_request_id;
...
begin
  select file_id into v_file_id from public.qa_refinery_distill where filename=p_filename limit 1;
  if coalesce(v_file_id,'')='' then raise exception 'QA_VECTOR_FILE_UNKNOWN'; end if;
  select decrypted_secret into v_key from vault.decrypted_secrets where name='qa_runner_publishable_key' limit 1;
  select decrypted_secret into v_op from vault.decrypted_secrets where name='vanhub_operator_token' limit 1;
  if coalesce(v_key,'')='' then raise exception 'QA_RUNNER_KEY_MISSING'; end if;
  if coalesce(v_op,'')='' then raise exception 'VANHUB_OPERATOR_KEY_MISSING'; end if;
  select net.http_post(
    url:='https://adpphssfoxpyzzonqofz.supabase.co/functions/v1/vanhub-vector-audit-fallback',
    body:=jsonb_build_object('file_id',v_file_id,'start',greatest(0,p_start),'limit',least(20000,greatest(1,p_limit))),
    headers:=jsonb_build_object('Content-Type','application/json','apikey',v_key,'x-vanhub-operator-token',v_op),
    timeout_milliseconds:=120000
  ) into v_request_id;
  return v_request_id;
