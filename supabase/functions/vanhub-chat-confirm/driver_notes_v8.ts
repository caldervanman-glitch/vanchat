...
export function reviewRiskNotes(summary:any){return list(summary?.quote_risks)}

export function driverRiskNotes(j:any,summary:any=null){
  return [multiStopNote(j),applianceDriverNote(j),...stateDriverNotes(j),...reviewRiskNotes(summary)].filter(Boolean)
}
