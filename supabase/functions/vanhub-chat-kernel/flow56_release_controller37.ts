...
const SINGULAR_ARTICLE=/\b(?:a|an|one|1)\s+[a-z][a-z0-9 -]{1,80}\b/i
...
function explicitSingleBusinessItem(j,candidate,message){
  ...
  const adds=(candidate?.inventory_add||[]).filter(x=>x&&['operational','approximate','correction'].includes(x.kind)&&clean(x.value)&&clean(x.evidence))
  if(adds.length!==1)return false
  const ev=clean(adds[0].evidence),value=clean(adds[0].value),m=canon(message)
  if(!ev||!value||!m.includes(canon(ev))||!SINGULAR_ARTICLE.test(ev))return false
  if(/\b(?:two|three|four|five|six|seven|eight|nine|ten|several|multiple|pair|pairs)\b/i.test(ev))return false
