...
function literalDateIso(v){
  ...
  let day,mon,year,explicitYear=false
  ...
  if(m){day=Number(m[1]);mon=MONTHS[m[2]];explicitYear=!!m[3];year=m[3]?Number(m[3]):null}
  else{
    ...
    if(!m)return null;mon=MONTHS[m[1]];day=Number(m[2]);explicitYear=!!m[3];year=m[3]?Number(m[3]):null
  }
  ...
  if(!explicitYear&&iso<today){year++;d=new Date(Date.UTC(year,mon,day));if(d.getUTCMonth()!==mon||d.getUTCDate()!==day)return null;iso=`${year}-${pad(mon+1)}-${pad(day)}`}
  return iso
}
function candidateDatePair(candidate,message){
  ...
  return {iso:String(iso.v),human:clean(human.v),expected:literalDateIso(human.v)}
}
function scrubMismatchedCandidateDate(j,j0,candidate,message){
  const p=candidateDatePair(candidate,message)
  if(!p?.expected||p.expected===p.iso)return false
  // An earlier controller may already have copied the model ISO into canonical
  // state. Remove it only when this turn introduced that exact candidate value;
  // never erase an unrelated pre-existing customer-grounded date.
  if(clean(j?.date?.iso_date)!==p.iso||clean(j0?.date?.iso_date)===p.iso)return false
  j.date??={original_text:null,iso_date:null,alternative_iso_dates:[],flexibility:null,time_preference:null}
  j.date.iso_date=null
  j.date.original_text=p.human
  j.date.alternative_iso_dates=[]
  if(j.q?.controller_date_ack_iso===p.iso)delete j.q.controller_date_ack_iso
  return true
}
function restoreExplicitCandidateDate(j,candidate,message){
  ...
  j.date??={original_text:null,iso_date:null,alternative_iso_dates:[],flexibility:null,time_preference:null}
  j.date.iso_date=p.iso;j.date.original_text=p.human;j.q??={};j.q.controller_date_ack_iso=p.iso
  return true
}

export function reduce(j0,f0,message,obj,candidate={},direct=null,media=[]){
  const r=base.reduce(j0,f0,message,obj,candidate,direct,media),j=r.j
  if(scrubMismatchedCandidateDate(j,j0,candidate,message)||restoreExplicitCandidateDate(j,candidate,message))r.f=requirements(j,r.f)
  return r
}
